import { Avatar, Button, Container,Text, HStack, Heading, Stack, VStack, Image, Modal, ModalOverlay, ModalContent, ModalCloseButton, ModalBody, Input, ModalFooter, useDisclosure, ModalHeader } from '@chakra-ui/react'
...
const Profile = ({user}) => {
    ...
    const {loading:subscriptionLoding,
    ...
    <HStack>
     ...
     {   user.role !== "admin" &&  (
        ...
        {user.subscription && user.subscription.status === "active"?(
            <Button onClick={cancelSubscriptionHandler} color={"purple.500"} isLoading={subscriptionLoding}>Cancil Subscription</Button>
        ):(
            <Link to="/subscribe">
...
function ChangePhotoBox({isOpen,onClose,changeImageSubmitHandler,loading}){
    ...
    return(
        <Modal isOpen={isOpen} onClose={closeHandler}>
           <ModalOverlay backdropFilter={"bulr(10px)"}/> 
           <ModalContent>
